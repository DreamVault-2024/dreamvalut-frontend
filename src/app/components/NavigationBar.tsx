...
const Playlist: React.FC<PlaylistProps> = ({ children }) => <div>{children}</div>;

const Mymusic: React.FC<MymusicProps> = ({ children }) => <div>{children}</div>;

const UserProfile: React.FC<UserProfileProps> = ({ children }) => <div>{children}</div>;
...
const NavBar: React.FC = () => {
    return (
      <div className="fixed left-0 top-0 h-full bg-black opacity-90 text-white p-4">
        ...
        <div className='flex items-center rounded-lg hover:bg-gray-900 cursor-pointer'>
        <div className='bg-gray-500 w-8 h-8 rounded-full'></div>
        <Playlist title="내 플레이리스트" songs={['노래 1', '노래 2', '노래 3']}>
        ...
        <button className='absolute bottom-0 right-0 w-20 h-10 hover-bg-gradient'>로그아웃</button>
        </LogOut>
        </div>
      </div>
    );
